...
const galleryHost: HTMLElement = galleryRoot;
const modalHost: HTMLElement = modalRoot;

const headerView = new HeaderView(headerRoot, events);
const galleryView = new GalleryView(galleryHost);
const modalView = new ModalView(modalHost, events);
...
function renderCatalog(): void {
  ...
  const cards = items.map((p) => {
    const card = new CatalogCardView(() => {
      events.emit(ViewEvents.CardSelect, { id: p.id });
    });
...
function renderPreview(product: IProduct): HTMLElement {
  const previewView = new PreviewCardView(() => {
    events.emit(ViewEvents.CardAction, { id: product.id });
  });
...
function renderBasket(): void {
  ...
  const rows = items.map((p, index) => {
    const row = new BasketCardView(() => {
      events.emit(ViewEvents.BasketItemRemove, { id: p.id });
    });
...
events.on<object>(ModelEvents.PreviewChanged, () => {
  ...
  openModal(renderPreview(product));
});
...
(async () => {
  try {
    ...
    galleryHost.replaceChildren(errorEl);
  }
})();